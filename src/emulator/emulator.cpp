...
void Emulator::run(std::string rom_path) {
    ...
    while (running) {
        ...
        arm7.step();
        // update textures
        SDL_UpdateTexture(top_texture, nullptr, gpu.engine_a.get_framebuffer(), sizeof(u32) * 9);
        SDL_UpdateTexture(bottom_texture, nullptr, gpu.engine_b.get_framebuffer(), sizeof(u32) * 9);
...
void Emulator::sdl_init() {
    ...
    top_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, 256, 384);
    bottom_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, 256, 384);
    
    // setup top and bottom texture dimensions
    top_texture_dimensions.x = 0;
    top_texture_dimensions.y = 0;
    top_texture_dimensions.w = 256;
    top_texture_dimensions.h = 192;

    bottom_texture_dimensions.x = 0;
    bottom_texture_dimensions.y = 192;
    bottom_texture_dimensions.w = 256;
    bottom_texture_dimensions.h = 192;
