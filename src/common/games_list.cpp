...
void GamesList::initialise() {
    ...
    // TODO: add cache file
    std::vector<std::string> paths = scan_directory("../roms", ".nds");
    ...
    std::sort(entries.begin(), entries.end(), [](Entry a, Entry b) {
        int result = to_lower(a.file_name).compare(to_lower(b.file_name));
        return result < 0;
    });
}
...
void GamesList::create_entry(const std::string& path) {
    ...
    entry.size = get_formatted_size(file.tellg());
