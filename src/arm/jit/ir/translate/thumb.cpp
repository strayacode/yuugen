...
Translator::BlockStatus Translator::thumb_branch_link_offset() {
    ...
    auto address = ir.add(ir.load_gpr(GPR::LR), ir.imm32(opcode.offset + (2 * instruction_size)));
    ir.link();
    ir.branch(address);
...
Translator::BlockStatus Translator::thumb_branch_link_setup() {
    ...
    auto address = ir.add(ir.load_gpr(GPR::PC), ir.imm32(opcode.imm));
...
Translator::BlockStatus Translator::thumb_branch_link_exchange_offset() {
    ...
    auto address = ir.add(ir.load_gpr(GPR::LR), ir.imm32(opcode.offset));
    ir.store_flag(Flag::T, ir.imm1(false));
...
Translator::BlockStatus Translator::thumb_branch() {
    ...
    auto instruction_size = ir.basic_block.location.get_instruction_size();
    auto address = ir.add(ir.load_gpr(GPR::PC), ir.imm32(opcode.offset + (2 * instruction_size)));
    ir.branch(address);
    return BlockStatus::Break;
}
...
Translator::BlockStatus Translator::thumb_push_pop() {
    ...
    if (opcode.pop) {
        ...
        if (opcode.pclr) {
            ...
            ir.store_gpr(GPR::SP, ir.add(address, ir.imm32(4)));
        ...
        for (int i = 0; i < 8; i++) {
            if (opcode.rlist & (1 << i)) {
                address = ir.subtract(address, ir.imm32(4));
            }
        }
        ...
        for (int i = 0; i < 8; i++) {
            if (opcode.rlist & (1 << i)) {
                ir.memory_write_word(address, ir.load_gpr(static_cast<GPR>(i)));
                address = ir.add(address, ir.imm32(4));
            }
        }

        if (opcode.pclr) {
            ir.memory_write_word(address, ir.load_gpr(GPR::LR));
        }
...
Translator::BlockStatus Translator::thumb_data_processing_register() {
    ...
    case ThumbDataProcessingRegister::Opcode::LSL: {
        auto pair = ir.barrel_shifter_logical_shift_left(op1, ir.truncate_byte(op2));
        auto result = TypedValue<Type::U32>{pair.first};
        ir.store_nz(result);
        ir.store_gpr(opcode.rd, result);
    ...
    case ThumbDataProcessingRegister::Opcode::LSR: {
        auto pair = ir.barrel_shifter_logical_shift_right(op1, ir.truncate_byte(op2));
        auto result = TypedValue<Type::U32>{pair.first};
        ir.store_nz(result);
        ir.store_gpr(opcode.rd, result);
        ir.store_flag(Flag::C, TypedValue<Type::U1>{pair.second});
    ...
    case ThumbDataProcessingRegister::Opcode::ASR: {
        auto pair = ir.barrel_shifter_arithmetic_shift_right(op1, ir.truncate_byte(op2));
        auto result = TypedValue<Type::U32>{pair.first};
        ir.store_nz(result);
        ir.store_gpr(opcode.rd, result);
        ir.store_flag(Flag::C, TypedValue<Type::U1>{pair.second});
    ...
    case ThumbDataProcessingRegister::Opcode::ROR: {
        auto pair = ir.barrel_shifter_rotate_right(op1, ir.truncate_byte(op2));
        auto result = TypedValue<Type::U32>{pair.first};
        ir.store_nz(result);
        ir.store_gpr(opcode.rd, result);
        ir.store_flag(Flag::C, TypedValue<Type::U1>{pair.second});
    ...
    case ThumbDataProcessingRegister::Opcode::NEG:
        result = ir.subtract(ir.imm32(0), op2);
        ir.store_nz(result);
        ir.store_sub_cv(ir.imm32(0), op2, result);
        break;
        break;
...
Translator::BlockStatus Translator::thumb_load_store_register_offset() {
    ...
    case ThumbLoadStoreRegisterOffset::Opcode::STR:
        ir.memory_write_word(address, ir.load_gpr(opcode.rd));
        break;
    case ThumbLoadStoreRegisterOffset::Opcode::STRB:
        ir.memory_write_byte(address, ir.truncate_byte(ir.load_gpr(opcode.rd)));
        break;
...
Translator::BlockStatus Translator::thumb_load_store_signed() {
    ...
    case ThumbLoadStoreSigned::Opcode::STRH:
        ir.memory_write_half(address, ir.truncate_half(ir.load_gpr(opcode.rd)));
        break;
...
Translator::BlockStatus Translator::thumb_load_pc() {
    ...
    auto address = ir.add(ir.bitwise_and(ir.load_gpr(GPR::PC), ir.imm32(~0x2)), ir.imm32(opcode.imm));
...
Translator::BlockStatus Translator::thumb_load_store_sp_relative() {
    ...
    auto address = ir.add(ir.load_gpr(GPR::SP), ir.imm32(opcode.imm << 2));
    if (opcode.load) {
        if (opcode.rd == 15) {
...
Translator::BlockStatus Translator::thumb_load_store_halfword() {
    ...
    auto address = ir.add(ir.load_gpr(opcode.rn), ir.imm32(opcode.imm << 1));
    if (opcode.load) {
        auto data = ir.memory_read(address, AccessSize::Half, AccessType::Aligned);
...
Translator::BlockStatus Translator::thumb_shift_immediate() {
    ...
    auto pair = ir.barrel_shifter(src, opcode.shift_type, ir.imm8(opcode.amount));
    auto result = TypedValue<Type::U32>{pair.first};
...
Translator::BlockStatus Translator::thumb_software_interrupt() {
    ...
    ir.store_gpr(GPR::PC, Mode::SVC, ir.imm32(exception_address + (4 * instruction_size)));
    return BlockStatus::Break;
}
...
Translator::BlockStatus Translator::thumb_load_store_multiple() {
    ...
    if (opcode.rlist == 0) {
        if (jit.arch == Arch::ARMv4) {
            if (opcode.load) {
                ...
                ir.memory_write_word(address, ir.load_gpr(GPR::PC));
            }
        }

        ir.store_gpr(opcode.rn, ir.add(address, ir.imm32(0x40)));
    ...
    if (opcode.load) {
        ...
        auto new_base = ir.add(address, ir.imm32(bytes));
        
        for (int i = 0; i < 8; i++) {
            if (opcode.rlist & (1 << i)) {
                if (i == opcode.rn && first != opcode.rn) {
                    ir.memory_write_word(address, new_base);
                } else {
                    ir.memory_write_word(address, ir.load_gpr(static_cast<GPR>(i)));
                }
...
Translator::BlockStatus Translator::thumb_load_store_immediate() {
    ...
    case ThumbLoadStoreImmediate::Opcode::STR:
        address = ir.add(address, ir.imm32(opcode.imm << 2));
        ir.memory_write_word(address, ir.load_gpr(opcode.rd));
        break;
    ...
    case ThumbLoadStoreImmediate::Opcode::STRB:
        address = ir.add(address, ir.imm32(opcode.imm));
        ir.memory_write_byte(address, ir.truncate_byte(ir.load_gpr(opcode.rd)));
        break;
...
Translator::BlockStatus Translator::thumb_add_sp_pc() {
    ...
    if (opcode.sp) {
        ir.store_gpr(opcode.rd, ir.add(ir.load_gpr(GPR::SP), ir.imm32(opcode.imm)));
    } else {
        ir.store_gpr(opcode.rd, ir.add(ir.bitwise_and(ir.load_gpr(GPR::PC), ir.imm32(~0x2)), ir.imm32(opcode.imm)));
    }
...
Translator::BlockStatus Translator::thumb_adjust_stack_pointer() {
    ...
    if (opcode.sub) {
        ir.store_gpr(GPR::SP, ir.subtract(ir.load_gpr(GPR::SP), ir.imm32(opcode.imm)));
    } else {
        ir.store_gpr(GPR::SP, ir.add(ir.load_gpr(GPR::SP), ir.imm32(opcode.imm)));
    }
