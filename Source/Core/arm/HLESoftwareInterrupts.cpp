...
void CPUCore::generate_software_interrupt_table() {
    ...
    if (arch == CPUArch::ARMv5) {
        ...
        register_software_interrupt(0x08, &CPUCore::software_interrupt_soundbias);
        register_software_interrupt(0x1B, &CPUCore::software_interrupt_get_pitch_table);
        register_software_interrupt(0x1C, &CPUCore::software_interrupt_get_volume_table);
        register_software_interrupt(0x1D, &CPUCore::software_interrupt_getbootprocs);
    }
    ...
    register_software_interrupt(0x06, &CPUCore::software_interrupt_halt);
    register_software_interrupt(0x0B, &CPUCore::software_interrupt_copy);
    register_software_interrupt(0x0C, &CPUCore::software_interrupt_fast_copy);
    register_software_interrupt(0x0E, &CPUCore::software_interrupt_get_crc16);
...
void CPUCore::software_interrupt_vblank_interrupt_wait() {
    ...
    software_interrupt_interrupt_wait();
}

void CPUCore::software_interrupt_interrupt_wait() {
    bool discard_old_flags = regs.r[0] == 1;
    u32 wait_flags = regs.r[1];
    halted = true;
    u32 address = 0;

    if (arch == CPUArch::ARMv5) {
        address = cp15->GetDTCMBase() + 0x3FF8;
    } else {
        address = 0x0380FFF8;
    }

    u32 flags = ReadWord(address);

    if (discard_old_flags) {
        if (flags & wait_flags) {
            WriteWord(address, flags & ~wait_flags);
        }
...
void CPUCore::software_interrupt_get_crc16() {
    u16 crc = regs.r[0];
    u32 start = regs.r[1] & ~0x1;
    int length = regs.r[2] & ~0x1;
    u16 values[] = {0xC0C1, 0xC181, 0xC301, 0xC601, 0xCC01, 0xD801, 0xF001, 0xA001};

    for (int i = 0; i < length; i++) {
        crc ^= ReadByte(start + i);
        ...
        for (int j = 0; j < 8; j++) {
            ...
            if (carry) {
                crc ^= static_cast<u32>(values[j]) << (7 - j);
            }
        }
    }

    // place calculated crc value in r0 for return value
    regs.r[0] = crc;
}

void CPUCore::software_interrupt_halt() {
    halted = true;
}

void CPUCore::software_interrupt_get_volume_table() {
    static u8 volume_table[] = {
    ...
    if (index > 0 && index < 0x2D4) {
        regs.r[0] = volume_table[index];
    } else {
        regs.r[0] = 0xAA;
    }
}

void CPUCore::software_interrupt_get_pitch_table() {
    ...
    if (index > 0 && index < 0x300) {
        regs.r[0] = pitch_table[index];
    } else {
        regs.r[0] = 0xAA;
    }
}

void CPUCore::software_interrupt_fast_copy() {
    u32 source = regs.r[0] & ~0x3;
    u32 destination = regs.r[1] & ~0x3;
    int count = (regs.r[2] & 0x1FFFFF) * 4;
    bool fixed = (regs.r[2] >> 24) & 0x1;
...
void CPUCore::software_interrupt_soundbias() {
    u32 bias_level = regs.r[0];
    
    if (bias_level) {
        WriteHalf(0x04000504, 0x200);
    } else {
        WriteHalf(0x04000504, 0);
    }
}

void CPUCore::software_interrupt_getbootprocs() {
