...
void SoftwareRenderer2D::render_extended(int bg, int line) {
    ...
    if (!(bgcnt[bg] & (1 << 7))) {
        ...
        for (int pixel = 0; pixel < 256; pixel++) {
            ...
            u8 palette_index = video_unit.vram.read_vram<u8>(character_addr);
        ...
        for (int pixel = 0; pixel < 256; pixel++) {
            ...
            u16 colour = video_unit.vram.read_vram<u16>(data_addr);
            bg_layers[bg][(256 * line) + pixel] = colour;
        }
    } else {
        // 256 colour bitmap
        for (int pixel = 0; pixel < 256; pixel++) {
            ...
            u8 palette_index = video_unit.vram.read_vram<u8>(data_addr);
            u16 colour = Common::read<u16>(palette_ram, (palette_index * 2) & 0x3FF);
