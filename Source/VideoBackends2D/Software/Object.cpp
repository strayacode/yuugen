...
void SoftwareRenderer2D::render_objects(int line) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        attribute[0] = Common::read<u16>(oam, i * 8);
        attribute[1] = Common::read<u16>(oam, (i * 8) + 2);
        attribute[2] = Common::read<u16>(oam, (i * 8) + 4);
        ...
        if (mode == 3) {
            ...
            for (int j = 0; j < width; j++) {
                ...
                u16 colour = video_unit.vram.read_vram<u16>(obj_base + ((height_difference * map_width) + j) * 2);

                if (colour & 0x8000) {
                    if (priority < obj_priority[(256 * line) + layer_offset]) {
                        obj_colour[(256 * line) + layer_offset] = colour;
                        obj_priority[(256 * line) + layer_offset] = priority;
                    }
                }
            }
        ...
        if (is_8bpp) {
            ...
            for (int j = 0; j < width; j++) {
                u16 colour = decode_obj_pixel_8bpp(obj_base, palette_number, j % 8, j / 8);
            ...
            // draw each pixel in the current row
            for (int j = 0; j < width; j++) {
                u16 colour = decode_obj_pixel_4bpp(obj_base, palette_number, j % 8, j / 8);
                u16 layer_offset = horizontal_flip ? x + width - j - 1 : x + j;
